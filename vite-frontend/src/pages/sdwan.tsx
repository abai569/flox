  ...
  const handleCreateGroup = async () => {
    ...
    try {
      ...
      if (res.code === 0) {
        ...
        setCreateForm({ name: "", networkCIDR: "", lighthouseNodeId: 0, memberNodeIds: [] });
        await loadData();
      } else {
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-4">
          {groups.map((group) => {
            ...
            return (
              ...
              >
                <div className="flex items-center gap-2 px-6 pt-4 pb-2 min-w-0">
                  <span className="font-semibold truncate">{group.name}</span>
                  <span className="bg-primary-500/10 text-primary-600 px-2 py-0.5 rounded text-xs flex-shrink-0">
                    {group.networkCIDR}
                  </span>
                </div>
                <CardBody>
                  <div className="space-y-1.5 border-b border-divider/50 pb-2 mb-2">
                    ...
                    <div className="flex justify-between items-center">
                      <span className="text-default-500 text-xs">成员数</span>
                      <span className="font-medium text-sm">{group.memberCount}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-default-500 text-xs">在线</span>
                      ...
                      >
                        <div className="flex items-center gap-2 min-w-0">
                          <span
                            className={`h-2.5 w-2.5 rounded-full flex-shrink-0 ${
                              member.status === 1 ? "bg-emerald-500" : "bg-rose-500"
                            }`}
                          />
                          <span className="text-sm font-medium truncate">{member.name}</span>
                          {member.role === "lighthouse" && (
                            <span className="px-1.5 py-0.5 rounded text-xs bg-warning-500/10 text-warning-600 flex-shrink-0">
                              中心
                            </span>
                          )}
                        </div>
                        <div className="flex items-center gap-3 flex-shrink-0 ml-2">
                          <span className="text-xs text-default-500 text-right">
                            <div>{member.vpnIp || "未分配"}</div>
                            {member.intranetIp && (
                              <div className="text-default-400">{member.intranetIp}</div>
                            )}
                          </span>
                          <span className="text-xs" title={member.overlayRunning ? "Overlay 运行中" : "Overlay 未运行"}>
                            {member.overlayRunning ? "🟢" : "🔴"}
                          </span>
                        </div>
                      </div>
                    ))}
              ...
              >
                <CardBody className="pt-4 pb-3 md:pt-4 space-y-1.5 w-full min-w-0">
                  <div className="flex items-center gap-2 min-w-0">
                    <span
                      className={`h-2.5 w-2.5 rounded-full flex-shrink-0 ${
                        node.status === 1 ? "bg-emerald-500" : "bg-rose-500"
                      }`}
                    />
                    <h3
              ...
              {nodes.map((node) => (
                <SelectItem key={String(node.id)} textValue={node.name}>
                  <div className="flex items-center justify-between">
                    <span>{node.name}</span>
                    <span className="text-xs">{node.status === 1 ? "🟢" : "🔴"}</span>
                  </div>
                </SelectItem>
              ))}
            </Select>
