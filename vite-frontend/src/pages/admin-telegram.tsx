      ...
      <Card>
        ...
        <CardBody className="pt-0">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
            <Input
              description="通过电报搜索 @BotFather 创建机器人后向机器人发送 /start 获取，格式如 123456:ABC-DEF..."
              isDisabled={isFree}
              label="Bot Token"
              placeholder="123456:ABC-DEF1234ghIkl-zyx..."
              value={config.bot_token}
              onChange={(e) =>
                setConfig((c) => ({ ...c, bot_token: e.target.value }))
              }
            />
            <Input
              description="通过电报搜索 @userinfobot 发送 /start 获取，格式如 Id: 1326......"
              isDisabled={isFree}
              label="Chat ID"
              placeholder="个人 ID 如 1326......"
              value={config.chat_id}
              onChange={(e) =>
                setConfig((c) => ({ ...c, chat_id: e.target.value }))
              }
            />
          </div>
            ...
            <div className="min-w-0">
              <div className="flex items-center gap-2">
                ...
                <span className="flex items-center gap-1.5">
                  <StatusDot
                    active={config.enabled && !isFree}
                    className="h-2 w-2"
                    tone={config.enabled && !isFree ? "success" : "danger"}
                  />
                  <span
      ...
      <Card className="mt-6 border border-divider shadow-sm">
        <CardHeader className="flex flex-col items-start gap-1 pb-4 pt-5 px-5">
          ...
          <p className="text-xs text-default-500">
            当以下事件触发时，面板将自动通过 Telegram Bot 发送实时通知
          </p>
        </CardHeader>
        <CardBody className="pt-0 pb-5 px-5">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4">
            ...
            />
            <NotificationItem
              desc="节点剩余流量不足 100G/50G/20G 时分别推送"
              icon="⚠️"
              title="节点流量告警"
            />
          </div>
