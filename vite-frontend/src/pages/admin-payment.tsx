...
interface PaymentStats {
  todayYipayPaid: number;
  todayUsdtPaid: number;
            ...
            <Card className="border border-gray-200 dark:border-default-200 shadow-md hover:shadow-lg transition-shadow">
              <CardBody className="p-3 lg:p-4">
                <div className="flex items-center justify-between mb-1">
                  <span className="text-xs text-default-500">今日易支付金额</span>
                  <div className="p-1.5 rounded-lg bg-green-100 dark:bg-green-500/20">
                    <svg className="w-4 h-4 text-green-600" fill="currentColor" viewBox="0 0 20 20">
                      ...
                      <path clipRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-13a1 1 0 10-2 0v.092a4.535 4.535 0 00-1.676.662C6.602 6.234 6 7.009 6 8c0 .99.602 1.765 1.324 2.246.48.32 1.054.545 1.676.662v1.941c-.391-.127-.68-.317-.843-.504a1 1 0 10-1.51 1.31c.562.649 1.413 1.076 2.353 1.253V15a1 1 0 102 0v-.092a4.535 4.535 0 001.676-.662C13.398 13.766 14 12.991 14 12c0-.99-.602-1.765-1.324-2.246A4.535 4.535 0 0011 9.092V7.151c.391.127.68.317.843.504a1 1 0 101.511-1.31c-.563-.649-1.413-1.076-2.354-1.253V5z" fillRule="evenodd" />
                    </svg>
                  </div>
                </div>
                <p className="text-xl font-bold text-green-600">{fmtMoney(stats.todayYipayPaid)} 元</p>
              </CardBody>
            </Card>
            <Card className="border border-gray-200 dark:border-default-200 shadow-md hover:shadow-lg transition-shadow">
              <CardBody className="p-3 lg:p-4">
                <div className="flex items-center justify-between mb-1">
                  <span className="text-xs text-default-500">今日U支付金额</span>
                ...
                <p className="text-xl font-bold text-cyan-600">{fmtMoney(stats.todayUsdtPaid)} 元</p>
              </CardBody>
            </Card>
            <Card className="border border-gray-200 dark:border-default-200 shadow-md hover:shadow-lg transition-shadow">
              <CardBody className="p-3 lg:p-4">
                <div className="flex items-center justify-between mb-1">
                  <span className="text-xs text-default-500">今日已付金额</span>
                ...
                <p className="text-xl font-bold text-violet-600">{fmtMoney(stats.todayTotalPaid)} 元</p>
              </CardBody>
            </Card>
            <Card className="border border-gray-200 dark:border-default-200 shadow-md hover:shadow-lg transition-shadow">
              <CardBody className="p-3 lg:p-4">
                <div className="flex items-center justify-between mb-1">
                  <span className="text-xs text-default-500">今日已付订单</span>
                ...
                <p className="text-xl font-bold text-emerald-600">{stats.todayPaidOrders}</p>
              </CardBody>
            </Card>
            <Card className="border border-gray-200 dark:border-default-200 shadow-md hover:shadow-lg transition-shadow">
              <CardBody className="p-3 lg:p-4">
                <div className="flex items-center justify-between mb-1">
                  <span className="text-xs text-default-500">待处理易支付</span>
                ...
                <p className="text-xl font-bold text-orange-600">{stats.pendingYipay}</p>
              </CardBody>
            </Card>
            <Card className="border border-gray-200 dark:border-default-200 shadow-md hover:shadow-lg transition-shadow">
              <CardBody className="p-3 lg:p-4">
                <div className="flex items-center justify-between mb-1">
                  <span className="text-xs text-default-500">待处理U支付</span>
                ...
                <p className="text-xl font-bold text-rose-600">{stats.pendingUsdt}</p>
              </CardBody>
            </Card>
          </div>
                    ...
                    {yipay.key && !editingYiPayKey ? (
                      <div className="h-10 flex items-center gap-2 px-3 border border-default-200 rounded-lg bg-gray-50 dark:bg-gray-900 text-sm">
                        <span className="truncate text-xs text-gray-600 dark:text-gray-400 font-mono">
                          {"\u2022".repeat(Math.max(0, yipay.key.length - 6))}
                          {yipay.key.slice(-6)}
                        </span>
                        <Button
                  ...
                  <div>
                    ...
                    <Select
                      ...
                      selectedKeys={usdt.network
                        ? usdt.network.split(",").filter(Boolean)
                        : []}
                    ...
                    >
                      {[
                        ...
                        { key: "binance", label: "Binance (BSC)" },
                        { key: "plasma", label: "Plasma" },
                        { key: "ton", label: "TON" },
                      ].map((net) => (
                        <SelectItem key={net.key}>{net.label}</SelectItem>
                      ))}
                    </Select>
                  </div>
                </div>
        ...
        <>
          ...
          {selectedLogIds.size > 0 && (
            <div className="flex items-center gap-2 mb-3">
              <span className="text-sm text-default-500 mr-1">
                已选 {selectedLogIds.size} 项
              </span>
              <Button
                color="danger"
                size="sm"
                variant="flat"
                onPress={handleBatchDeleteLogs}
              >
                批量删除
              </Button>
            </div>
                  ...
                  logs.map((log) => (
                    <TableRow key={log.id}>
                      ...
                      <TableCell className="whitespace-nowrap">
                        {userRemarkMap.has(log.userId)
                          ? `${userRemarkMap.get(log.userId)}@`
                          : ""}
                        {log.userName}
                      </TableCell>
                      <TableCell
