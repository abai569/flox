      ...
      <TableBody>
        {items.map((item) => (
          <TableRow key={item.id}>
            ...
            <TableCell>
              <Button size="sm" variant="flat" onPress={() => onDescEdit(item)}>
                编辑
              </Button>
            </TableCell>
            <TableCell>
            ...
            <TableCell>
              <div className="text-sm">{item.validityDays}天</div>
            </TableCell>
            <TableCell>
            ...
            <TableCell className="text-xs text-gray-500">
              <div className="space-y-0.5">
                <div>
                  ...
                  {item.trafficLimit > 0 ? `${item.trafficLimit} GB` : "不限"}
                </div>
                <div>
                  连接 {item.maxConnections > 0 ? item.maxConnections : "不限"}{" "}
                  · 限速{" "}
                  {item.speedLimit > 0 ? `${item.speedLimit} Mbps` : "不限"}
                </div>
              </div>
            </TableCell>
            <TableCell>
              <div className="flex flex-row gap-1 shrink-0">
                {renderStatusBadge(item.enabled)}
              </div>
            </TableCell>
            <TableCell>
              <div className="flex flex-row gap-1 shrink-0">
                {renderShopVisibleBadge(item.shopVisible)}
              </div>
            </TableCell>
            <TableCell>
              <div className="flex flex-row gap-1 shrink-0">
            ...
            <TableCell>
              <div className="flex flex-row gap-1 shrink-0">
                {renderRecommendedBadge(item.recommended)}
              </div>
            </TableCell>
            <TableCell>
              <div className="text-sm">{renderStock(item.stock)}</div>
            </TableCell>
      ...
      {groupedPackages.map(({ group, items }) => {
        ...
        return (
          ...
          >
            ...
            {!isCollapsed && (
              <div className="overflow-x-auto">{renderGroupTable(items)}</div>
            )}
          </div>
        );
      })}
