...
func TestValidatePeerShareScopeRejectsForeignInstance(t *testing.T) {
	r, err := repo.Open(filepath.Join(t.TempDir(), "scope.db"))
	if err != nil {
	...
	for _, item := range []struct {
		nodeID     int64
		instanceID string
	}{{1, "instance-a"}, {2, "instance-b"}} {
		if err := r.DB().Create(&repo.Node{ID: item.nodeID, Name: item.instanceID, Secret: item.instanceID, ServerIP: "127.0.0.1", Port: "30000-30010", CreatedTime: now, Status: 1, TCPListenAddr: "[::]", UDPListenAddr: "[::]"}).Error; err != nil {
			t.Fatalf("create node: %v", err)
		}
		if err := r.UpsertNodeInstance(repo.NodeInstanceUpsert{NodeID: item.nodeID, InstanceID: item.instanceID, Now: now}); err != nil {
			t.Fatalf("create instance: %v", err)
		}
	}
	if _, _, _, _, err := h.validatePeerShareScope(1, "selected", nil, 1, []string{"instance-b"}); err == nil {
		t.Fatalf("expected foreign instance rejection")
	}
}

func TestScopedPeerShareInstancesFallsBackForLegacyNode(t *testing.T) {
	r, err := repo.Open(filepath.Join(t.TempDir(), "legacy-scope.db"))
	if err != nil {
		t.Fatalf("open repo: %v", err)
	}
	t.Cleanup(func() { _ = r.Close() })
	h := &Handler{repo: r}
	instances, legacy, err := h.scopedPeerShareInstances(&repo.PeerShare{ID: 1, NodeID: 1, ScopeType: "all_enabled", AutoIncludeNewInstances: 1})
	if err != nil {
		t.Fatalf("resolve legacy scope: %v", err)
	}
	if !legacy || len(instances) != 0 {
		t.Fatalf("expected legacy fallback, legacy=%v instances=%+v", legacy, instances)
	}
}

func TestValidatePeerShareScopeRejectsLegacyMinimumAboveOne(t *testing.T) {
	r, err := repo.Open(filepath.Join(t.TempDir(), "legacy-minimum.db"))
	if err != nil {
		t.Fatalf("open repo: %v", err)
	}
	t.Cleanup(func() { _ = r.Close() })
	...
	h := &Handler{repo: r}
	if _, _, _, _, err := h.validatePeerShareScope(1, "all_enabled", nil, 2, nil); err == nil {
		t.Fatalf("expected legacy single-instance scope to reject minimum above one")
	}
}
...
func TestFederationTunnelCreateRejectsOccupiedPort(t *testing.T) {
	...
	create := func() response.R {
		...
		req := httptest.NewRequest(http.MethodPost, "/api/v1/federation/tunnel/create", bytes.NewReader(body))
		req.Header.Set("Authorization", "federation-port-check-token")
		req.Header.Set("Content-Type", "application/json")
		res := httptest.NewRecorder()
		h.federationTunnelCreate(res, req)
		if res.Code != http.StatusOK {
