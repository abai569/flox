...
func (h *Handler) paymentStats(w http.ResponseWriter, r *http.Request) {
	todayYipayPaid, todayUsdtPaid, todayTotalPaid, todayPaidOrders, pendingYipay, pendingUsdt, err := h.repo.GetTodayPaymentStats()
	if err != nil {
		response.WriteJSON(w, response.Err(-2, err.Error()))
		return
	}
	response.WriteJSON(w, response.OK(map[string]int64{
		"todayYipayPaid":  todayYipayPaid,
		"todayUsdtPaid":   todayUsdtPaid,
		"todayTotalPaid":  todayTotalPaid,
		"todayPaidOrders": todayPaidOrders,
		"pendingYipay":    pendingYipay,
...
func (h *Handler) completePayment(orderNo, txHash string) {
	...
	if txHash != "" {
		if err := h.repo.UpdateOrderTxHash(order.ID, txHash); err != nil {
			log.Printf("[completePayment] update tx hash failed for order %s: %v", orderNo, err)
		}
	}
