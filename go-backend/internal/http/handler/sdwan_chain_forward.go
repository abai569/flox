...
func (h *Handler) fillSDWANChainNodeConnectIPs(chainNodes []chainNodeRecord) {
	for i := range chainNodes {
		...
		// 优先从分组配置取 VPN IP（选证书有效的分组）
		groupID, groupCfg := h.findBestSDWANGroup(node.RemoteConfig)
		vpnIP := ""
		if groupID != "" && groupCfg != nil {
...
// findBestSDWANGroup 遍历所有分组，选择证书有效且剩余时间最长的。
// 用于避免多面板时取到其他面板的过期分组。
func (h *Handler) findBestSDWANGroup(remoteConfig string) (string, map[string]interface{}) {
	cfg := parseRemoteConfigMap(remoteConfig)
	groupsRaw, ok := cfg["sdwanGroups"]
	if !ok {
		return "", nil
	...
	var bestID string
	var bestCfg map[string]interface{}
	var bestExpiry time.Time

	for gid, gRaw := range groupsMap {
		gMap, ok := gRaw.(map[string]interface{})
		if !ok {
			continue
		}
		certPEM, ok := gMap["certPEM"].(string)
		if !ok || certPEM == "" {
			continue
		}
		...
		// 选择剩余时间最长的
		if bestID == "" || cert.Details.NotAfter.After(bestExpiry) {
			bestID = gid
			bestCfg = gMap
			bestExpiry = cert.Details.NotAfter
		}
	}
	...
	if bestID == "" {
		for gid, gRaw := range groupsMap {
			if gMap, ok := gRaw.(map[string]interface{}); ok {
				return gid, gMap
			}
		}
	}
...
func (h *Handler) buildSDWANMetadataForNode(node *nodeRecord, dialMode string, overlayListen bool) map[string]interface{} {
	...
	// 优先从分组配置中获取（新逻辑）
	// 遍历所有分组，选择证书有效且剩余时间最长的
	groupID, groupCfg := h.findBestSDWANGroup(node.RemoteConfig)
	if groupID != "" && groupCfg != nil {
		fmt.Printf("[sdwan.debug] buildMetadata: node=%s groupID=%s hasCAPEM=%v hasCertPEM=%v\n",
			node.Name, groupID,
			groupCfg["caPEM"] != nil && groupCfg["caPEM"].(string) != "",
			groupCfg["certPEM"] != nil && groupCfg["certPEM"].(string) != "")
