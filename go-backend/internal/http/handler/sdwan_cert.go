...
func (h *Handler) syncSDWANLighthouseTopology() error {
	...
	// 2 节点模式：没有中心节点，让入口节点充当 lighthouse
	// 找出所有参与 SDWAN 的节点（有 SDWAN 配置的节点）
	sdwanNodeIDs := make(map[int64]bool)
	var entryNodeID int64
	for _, item := range items {
		nodeID := asInt64(item["id"], 0)
		remoteConfig := asString(item["remoteConfig"])
		if strings.Contains(remoteConfig, "sdwanCAPEM") || strings.Contains(remoteConfig, "sdwanCertPEM") {
			sdwanNodeIDs[nodeID] = true
			// 找入口节点（ChainType=0 或 1 的节点，通常是第一个有 SDWAN 配置的节点）
			if entryNodeID == 0 {
				entryNodeID = nodeID
			}
		}
	...
	// 如果没有配置 primary lighthouse，且有 SDWAN 节点，让入口节点充当 lighthouse
	if primary == nil && entryNodeID > 0 {
		for _, item := range items {
			nodeID := asInt64(item["id"], 0)
			if nodeID == entryNodeID {
				...
				// 获取节点的公网 IP 作为 lighthouse 地址
				node, err := h.repo.GetNodeByID(nodeID)
				if err != nil {
					continue
				}
				publicIP := strings.TrimSpace(node.ServerIP)
				if publicIP == "" && node.ServerIPV4.Valid {
					publicIP = strings.TrimSpace(node.ServerIPV4.String)
				}
				if publicIP == "" && node.ServerIPV6.Valid {
					publicIP = strings.TrimSpace(node.ServerIPV6.String)
				}
				listenPort := parseSDWANListenPortFromRemoteConfig(remoteConfig)
				if listenPort == "" {
					listenPort = "4242"
				}
				lighthouseAddr := net.JoinHostPort(strings.Trim(publicIP, "[]"), listenPort)

				updated := mergeRemoteConfig(remoteConfig, map[string]string{
					"sdwanIsLighthouse":           "true",
					"sdwanLighthouseRole":         "primary",
					"sdwanLighthouseVPNIP":        vpnIP,
					"sdwanLighthouseAddr":         lighthouseAddr,
					"sdwanBackupLighthouseVPNIPs": "",
					"sdwanBackupLighthouseAddrs":  "",
				})
				if err := h.repo.UpdateNodeRemoteConfig(nodeID, updated); err != nil {
					return err
				}
			} else if sdwanNodeIDs[nodeID] {
				// 其他 SDWAN 节点作为 peer
				remoteConfig := asString(item["remoteConfig"])
				entryNode, err := h.repo.GetNodeByID(entryNodeID)
				if err != nil {
					continue
				}
				entryRemoteConfig := entryNode.RemoteConfig.String
				entryVPNIP := parseSDWANNodeVPNIPFromRemoteConfig(entryRemoteConfig)
				if entryVPNIP == "" {
					entryVPNIP = "192.168.100.1"
				}
				entryPublicIP := strings.TrimSpace(entryNode.ServerIP)
				if entryPublicIP == "" && entryNode.ServerIPV4.Valid {
					entryPublicIP = strings.TrimSpace(entryNode.ServerIPV4.String)
				}
				if entryPublicIP == "" && entryNode.ServerIPV6.Valid {
					entryPublicIP = strings.TrimSpace(entryNode.ServerIPV6.String)
				}
				entryListenPort := parseSDWANListenPortFromRemoteConfig(entryRemoteConfig)
				if entryListenPort == "" {
					entryListenPort = "4242"
				}
				entryLighthouseAddr := net.JoinHostPort(strings.Trim(entryPublicIP, "[]"), entryListenPort)

				updated := mergeRemoteConfig(remoteConfig, map[string]string{
					"sdwanIsLighthouse":           "false",
					"sdwanLighthouseRole":         "peer",
					"sdwanLighthouseVPNIP":        entryVPNIP,
					"sdwanLighthouseAddr":         entryLighthouseAddr,
					"sdwanBackupLighthouseVPNIPs": "",
					"sdwanBackupLighthouseAddrs":  "",
				})
				if err := h.repo.UpdateNodeRemoteConfig(nodeID, updated); err != nil {
...
func (h *Handler) nodeSDWANUnbootstrap(w http.ResponseWriter, r *http.Request) {
	var req struct {
		NodeID int64 `json:"nodeId"`
	}
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
		response.WriteJSON(w, response.Err(1, "无效的请求参数"))
		return
	}
	...
	if err := h.repo.UpdateNodeRemoteConfig(node.ID, updatedRemoteConfig); err != nil {
		response.WriteJSON(w, response.Err(1, "保存配置失败"))
		return
	}
	if h.wsServer != nil {
		h.wsServer.SendCommand(req.NodeID, "SdwanShutdown", nil, 10*time.Second)
	}
