...
func TestFederationRuntimeResourceKeySeparatesDeployments(t *testing.T) {
	first := federationRuntimeResourceKey("deploy-a", 7, 9, 2, 1)
	second := federationRuntimeResourceKey("deploy-b", 7, 9, 2, 1)
	if first == second {
		t.Fatalf("expected different deployments to have different resource keys: %q", first)
	}
	if got := federationRuntimeResourceKey("deploy-a", 7, 9, 2, 1); got != first {
		t.Fatalf("expected resource key to be stable within one deployment, got %q and %q", first, got)
	}
}

func TestReplaceFederationBindingsIfResourceKeysMatchProtectsNewerDeployment(t *testing.T) {
	r, err := repo.Open(filepath.Join(t.TempDir(), "binding-owner.db"))
	if err != nil {
	...
	tunnel := model.Tunnel{Name: "binding-owner", TrafficRatio: 1, Type: 2, Protocol: "tls", Flow: 1, CreatedTime: now, UpdatedTime: now, Status: 1}
	if err := r.DB().Create(&tunnel).Error; err != nil {
		t.Fatalf("create tunnel: %v", err)
	}
	newBinding := &repo.FederationTunnelBinding{
		TunnelID: tunnel.ID, NodeID: 9, ChainType: 3, RemoteURL: "http://new", ResourceKey: "new-key",
		RemoteBindingID: "new-binding", AllocatedPort: 32001, Status: 1, CreatedTime: now, UpdatedTime: now,
	}
	if err := r.UpsertFederationTunnelBinding(newBinding); err != nil {
		t.Fatalf("create new binding: %v", err)
	}
	oldBindings := []repo.FederationTunnelBinding{{
		TunnelID: tunnel.ID, NodeID: 9, ChainType: 3, RemoteURL: "http://old", ResourceKey: "old-key",
		RemoteBindingID: "old-binding", AllocatedPort: 31001, Status: 1, CreatedTime: now - 1, UpdatedTime: now - 1,
	}}

	replaced, err := r.ReplaceFederationTunnelBindingsIfResourceKeysMatch(tunnel.ID, now, []string{"stale-key"}, oldBindings)
	if err != nil {
		t.Fatalf("stale conditional replace: %v", err)
	}
	if replaced {
		t.Fatalf("stale deployment must not replace newer bindings")
	}
	replaced, err = r.ReplaceFederationTunnelBindingsIfResourceKeysMatch(tunnel.ID, now-1, []string{"new-key"}, oldBindings)
	if err != nil {
		t.Fatalf("stale version conditional replace: %v", err)
	}
	if replaced {
		t.Fatalf("stale tunnel version must not replace current bindings")
	}
	bindings, err := r.ListActiveFederationTunnelBindingsByTunnel(tunnel.ID)
	if err != nil || len(bindings) != 1 || bindings[0].ResourceKey != "new-key" {
		t.Fatalf("expected newer binding to remain, got %+v (err=%v)", bindings, err)
	}

	replaced, err = r.ReplaceFederationTunnelBindingsIfResourceKeysMatch(tunnel.ID, now, []string{"new-key"}, oldBindings)
	if err != nil {
		t.Fatalf("owned conditional replace: %v", err)
	}
	if !replaced {
		t.Fatalf("current deployment should be allowed to restore old bindings")
	}
	bindings, err = r.ListActiveFederationTunnelBindingsByTunnel(tunnel.ID)
	if err != nil || len(bindings) != 1 || bindings[0].ResourceKey != "old-key" {
		t.Fatalf("expected old binding to be restored, got %+v (err=%v)", bindings, err)
	}
}

func TestCompleteFederationDeploymentPublishesOnlyCurrentTunnelVersion(t *testing.T) {
	r, err := repo.Open(filepath.Join(t.TempDir(), "binding-complete.db"))
	if err != nil {
		t.Fatalf("open repo: %v", err)
	}
	defer r.Close()
	...
	now := time.Now().UnixMilli()
	tunnel := model.Tunnel{Name: "binding-complete", TrafficRatio: 1, Type: 2, Protocol: "tls", Flow: 1, CreatedTime: now, UpdatedTime: now, Status: 1}
	if err := r.DB().Create(&tunnel).Error; err != nil {
		t.Fatalf("create tunnel: %v", err)
	}
	oldBinding := &repo.FederationTunnelBinding{
		TunnelID: tunnel.ID, NodeID: 9, ChainType: 3, RemoteURL: "http://old", ResourceKey: "old-key",
		RemoteBindingID: "old-binding", AllocatedPort: 31001, Status: 1, CreatedTime: now, UpdatedTime: now,
	}
	if err := r.UpsertFederationTunnelBinding(oldBinding); err != nil {
		t.Fatalf("create old binding: %v", err)
	}
	newBindings := []repo.FederationTunnelBinding{{
		TunnelID: tunnel.ID, NodeID: 9, ChainType: 3, RemoteURL: "http://new", ResourceKey: "new-key",
		RemoteBindingID: "new-binding", AllocatedPort: 32001, Status: 1, CreatedTime: now + 1, UpdatedTime: now + 1,
	}}

	previous, switched, err := r.CompleteFederationTunnelDeployment(tunnel.ID, now-1, newBindings)
	if err != nil {
		t.Fatalf("complete stale deployment: %v", err)
	}
	if switched || len(previous) != 0 {
		t.Fatalf("stale deployment must not publish or claim old bindings: switched=%v previous=%+v", switched, previous)
	}

	previous, switched, err = r.CompleteFederationTunnelDeployment(tunnel.ID, now, newBindings)
	if err != nil {
		t.Fatalf("complete current deployment: %v", err)
	}
	if !switched || len(previous) != 1 || previous[0].ResourceKey != "old-key" {
		t.Fatalf("expected atomic switch to return old binding, switched=%v previous=%+v", switched, previous)
	}
	bindings, err := r.ListActiveFederationTunnelBindingsByTunnel(tunnel.ID)
	if err != nil || len(bindings) != 1 || bindings[0].ResourceKey != "new-key" {
		t.Fatalf("expected new binding to be published, got %+v (err=%v)", bindings, err)
	}
}
...
func TestFederationRuntimeReservePortRejectsWhenShareFlowExceeded(t *testing.T) {
	...
	req.Header.Set("Authorization", "limited-token")
