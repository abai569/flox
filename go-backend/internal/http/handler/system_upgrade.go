...
func (e *systemUpgradeExecutor) helperScript() string {
	return strings.Join([]string{
		...
		`  NEW_BACKEND_IMAGE=$(docker inspect -f '{{.Image}}' flox-svc-backend 2>/dev/null || true)`,
		`  [ -n "$NEW_BACKEND_IMAGE" ] || return 0`,
		`  CLEANUP_SCRIPT='sleep 15`,
		...
		`  docker image ls --format "{{.Repository}}:{{.Tag}}" "$IMAGE_NAME" 2>/dev/null | while IFS= read -r IMAGE; do`,
		`    [ -n "$IMAGE" ] || continue`,
		...
		`  done`,
		`done`,
		`docker image rm -f "flox-upgrade-backend-rollback:${UPGRADE_ID}" "flox-upgrade-frontend-rollback:${UPGRADE_ID}" >/dev/null 2>&1 || true'`,
		`  docker run -d --rm --name "FLOX-upgrade-cleanup-${UPGRADE_ID}" -v /var/run/docker.sock:/var/run/docker.sock -e TARGET_VERSION="$TARGET_VERSION" -e UPGRADE_ID="$UPGRADE_ID" --entrypoint /bin/sh "$NEW_BACKEND_IMAGE" -c "$CLEANUP_SCRIPT" >/dev/null 2>&1 || true`,
		`}`,
