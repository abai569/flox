...
func (b *Bot) sendMessage(text string) error {
	...
	resp, err := b.client.httpClient.Post(url, "application/json", bytes.NewReader(body))
	if err != nil {
...
func (b *Bot) SendNodeOffline(nodeName string) {
	b.SendAlert("节点离线", fmt.Sprintf("节点 %s 已断开（可能是网络抖动）", nodeName))
}

func (b *Bot) SendNodeStillOffline(nodeName string, minutes int) {
	b.SendAlert("节点持续离线", fmt.Sprintf("节点 %s 已离线超过 %d 分钟，仍未恢复", nodeName, minutes))
}
...
func (b *Bot) SendNodeTrafficReset(nodeName, reason string) {
...
func (b *Bot) SendNodeTrafficAlert(nodeName string, remainingGB, thresholdGB int64) {
	b.SendAlert("⚠️ 节点流量告警",
		fmt.Sprintf("节点 %s 剩余流量不足 %dGB\n当前剩余: %dGB / 阈值: %dGB",
			nodeName, thresholdGB, remainingGB, thresholdGB))
}

func (b *Bot) SendNodeTrafficExceeded(nodeName string) {
	b.SendAlert("🚫 节点流量已用完", fmt.Sprintf("节点 %s 流量已达到限制，已被暂停，请联系管理员归零", nodeName))
}
