...
func (r *Repository) UpdateOrderTxHash(id int64, txHash string) error {
	if r == nil || r.db == nil {
		return errors.New("repository not initialized")
	}
...
func (r *Repository) GetTodayPaymentStats() (todayYipayPaid int64, todayUsdtPaid int64, todayTotalPaid int64, todayPaidOrders int64, pendingYipay int64, pendingUsdt int64, err error) {
	if r == nil || r.db == nil {
		return 0, 0, 0, 0, 0, 0, errors.New("repository not initialized")
	}

	now := time.Now()
	todayStart := time.Date(now.Year(), now.Month(), now.Day(), 0, 0, 0, 0, now.Location()).Unix()

	r.db.Model(&model.Order{}).Where("status = 1 AND pay_currency = 'YIPAY' AND pay_time >= ?", todayStart).Select("COALESCE(SUM(amount),0)").Scan(&todayYipayPaid)
	r.db.Model(&model.Order{}).Where("status = 1 AND pay_currency = 'USDT' AND pay_time >= ?", todayStart).Select("COALESCE(SUM(amount),0)").Scan(&todayUsdtPaid)
	r.db.Model(&model.Order{}).Where("status = 1 AND pay_time >= ?", todayStart).Select("COALESCE(SUM(amount),0)").Scan(&todayTotalPaid)
	r.db.Model(&model.Order{}).Where("status = 1 AND pay_time >= ?", todayStart).Count(&todayPaidOrders)
	r.db.Model(&model.Order{}).Where("status = 0 AND pay_currency = 'YIPAY'").Count(&pendingYipay)
	r.db.Model(&model.Order{}).Where("status = 0 AND pay_currency = 'USDT'").Count(&pendingUsdt)
	return
}
