...
func (w *WebSocketReporter) handleAddNftablesRules(data json.RawMessage) error {
	...
	for _, rule := range req.Rules {
		...
		if rule.NextHopIPv6 != "" {
			...
			if err := w.nftablesMgr.AddRule(rule.ForwardID, rule.NodeID, rule.UserID, rule.UserTunnelID, rule.Protocol, rule.Port, ipv6Target, rule.SpeedLimit); err != nil {
...
func (w *WebSocketReporter) handleUpdateNftablesRules(data json.RawMessage) error {
	...
	for _, rule := range req.Rules {
		...
		if rule.NextHopIPv6 != "" {
			ipv6Target := net.JoinHostPort(rule.NextHopIPv6, strconv.Itoa(rule.NextHopPort))
			if err := w.nftablesMgr.UpdateRule(rule.ForwardID, rule.Protocol, rule.Port, ipv6Target, rule.SpeedLimit); err != nil {
