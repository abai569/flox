...
func serviceKernelDebugLabel(cfg *config.ServiceConfig) string {
	if cfg == nil {
		return ""
	}
	if cfg.Metadata != nil {
...
func serviceHandlerType(cfg *config.ServiceConfig) string {
	if cfg == nil || cfg.Handler == nil {
		return ""
	}
	return strings.TrimSpace(strings.ToLower(cfg.Handler.Type))
}
...
func parseFloxService(cfg *config.ServiceConfig) (service.Service, error) {
	kernel := kernelName(cfg)
	requestedKernel := serviceKernelDebugLabel(cfg)
	handlerType := serviceHandlerType(cfg)
	serviceName := ""
	serviceAddr := ""
	mimicRole := ""
	if cfg != nil {
		...
		if cfg.Metadata != nil {
			if v, ok := cfg.Metadata["mimicRole"]; ok {
				if s, ok := v.(string); ok {
					mimicRole = strings.TrimSpace(s)
				}
			}
		}
	}
	if requestedKernel != "" || kernel != "gost" {
		fmt.Printf("[kernel.debug] parseFloxService name=%s addr=%s handler=%s requested=%s resolved=%s\n", serviceName, serviceAddr, handlerType, requestedKernel, kernel)
	}
	...
	case "floxcore":
		...
		default:
			return nil, fmt.Errorf("floxcore kernel does not support handler type %s", handlerType)
		}
	case "sdwan":
		switch handlerType {
		case "tcp", "udp":
		...
		default:
			return nil, fmt.Errorf("sdwan kernel does not support handler type %s", handlerType)
		}
	case "mimic":
		fmt.Printf("[mimic.debug] kernel route hit name=%s addr=%s handler=%s role=%s\n", serviceName, serviceAddr, handlerType, mimicRole)
		switch handlerType {
		case "tcp", "udp":
			sfc := toFloxConfig(cfg)
			return adapter.NewMimicForwardService(sfc)
		default:
			return nil, fmt.Errorf("mimic kernel does not support handler type %s", handlerType)
		}
	}
	if requestedKernel == "mimic" && kernel == "gost" {
		fmt.Printf("[mimic.debug] requested mimic kernel fell back to gost name=%s addr=%s handler=%s\n", serviceName, serviceAddr, handlerType)
	}
	return nil, nil
}
